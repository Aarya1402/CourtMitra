...
const OrderForm: React.FC<Props> = ({ data, onUpdate, isProcessing }) => {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      clone.querySelectorAll("button").forEach((btn) => (btn as HTMLElement).style.display = "none");
      ...
      <div className={styles.formContainer}>
        ...
        <div className={styles.documentPage} ref={containerRef}>
          ...
          <div className={styles.reasoningSection}>
            <h5 style={{ textDecoration: "underline", fontSize: "14pt", marginBottom: "15px" }}>Reasoning / Analysis (મુદ્દાઓ):</h5>
            {Array.isArray(formData.reasoning_points) && formData.reasoning_points.map((p, idx) => (
              <div key={idx} className={styles.reasoningPoint}>
                <span style={{ fontWeight: "bold", width: "30px" }}>{idx + 1}.</span>
                <div style={{ flex: 1 }}>
                  {renderTextArea(p, ["reasoning_points", idx.toString()], `Point ${idx + 1}`)}
                </div>
                <button 
            ...
            <button 
              ...
              onClick={() => onUpdate({ ...formData, reasoning_points: [...(formData.reasoning_points || []), ""] })}
            >
              + Add Reasoning Point
            </button>
          </div>
          ...
          <div className={styles.orderContent}>
            ...
            <div className={styles.directionsList}>
              <h5 style={{ marginTop: "15px" }}>Specific Directions / Conditions:</h5>
              {Array.isArray(formData.operative_order?.directions) && formData.operative_order.directions.map((d, idx) => (
                <div key={idx} className={styles.directionPoint}>
                  <span style={{ fontWeight: "bold" }}>({idx + 1})</span>
                  <div style={{ flex: 1 }}>
                    {renderTextArea(d, ["operative_order", "directions", idx.toString()], `Direction ${idx + 1}`)}
                  </div>
                  <button 
                    className={styles.removeBtn} 
                    onClick={() => {
                      ...
                      next.splice(idx, 1);
                      onUpdate({ ...formData, operative_order: { ...formData.operative_order, directions: next } });
                    }}
              ...
              <button 
                ...
                onClick={() => {
                  ...
                  onUpdate({ ...formData, operative_order: { ...formData.operative_order, directions: [...current, ""] } });
                }}
              >
                + Add Direction
              </button>
