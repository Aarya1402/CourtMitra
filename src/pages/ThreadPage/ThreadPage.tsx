...
const ThreadPage: React.FC = () => {
  ...
  const extractDataFromChunk = async (fullTranscript: string) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://${window.location.hostname}:5000/api/gemini/extract`,
        data,
      );
      ...
      let parsedObj = Array.isArray(newJson) ? newJson[0] : newJson;
  ...
  const fetchThreadTitle = async () => {
    ...
    try {
      const threadsRes = await axios.get(
        `http://${window.location.hostname}:5000/api/talkument/bots/thread/${threadId}`,
      );
  ...
  const fetchChatHistory = async () => {
    ...
    try {
      const chatHistory = await axios.get(
        `http://${window.location.hostname}:5000/api/talkument/bots/${threadId}/chat_history?page=1&per_page=30`,
      );
      ...
      if (botId) {
        try {
          const threadsRes = await axios.get(
            `http://${window.location.hostname}:5000/api/talkument/bots/${botId}/threads?page=1&per_page=100`,
          );
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      const responseFile = await axios.get(
        `http://${window.location.hostname}:5000/api/talkument/bots/file/${threadId}/status`,
      );
      ...
      const response = await fetch(
        `http://${window.location.hostname}:5000/api/talkument/bots/agui/interact/${threadId}`,
        {
          method: "POST",
          headers: {
  ...
  const handleDelete = async (messageId: string) => {
    try {
      await axios.delete(
        `http://${window.location.hostname}:5000/api/talkument/bots/chat/${messageId}/delete`,
      );
